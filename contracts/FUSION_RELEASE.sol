...
contract FUSION_RELEASE is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address=>Investor) public _investor;
    ...
    // function changeConfig(uint256 openStart_,uint256 refundHour_,uint256 lockTime_,uint256 maxOrderCount_,uint256 priorityTime_,uint256 orderAmountLimit_,address collect_,address manager_) external onlyOwner {
    //     _openStart = openStart_;
    //     _refundHour = refundHour_;
    //     _lockTime = lockTime_;
    //     _maxOrderCountForDay = maxOrderCount_;
    //     _priorityTime = priorityTime_;
    ...
    function changeConfig2(uint256 lockTime_,uint256 maxOrderCount_,uint256 orderAmountLimit_,address collect_,address manager_) external onlyOwner {
        _lockTime = lockTime_;
        _maxOrderCountForDay = maxOrderCount_;
        _orderAmountLimit = orderAmountLimit_;
        _collect = collect_;
    ...
    // function changeRefundHour(uint256 lockTime_,uint256 refundHour_) external onlyOwner {
    //     _refundHour = refundHour_;
    //     _lockTime = lockTime_;
    // }
    ...
    function execRefund(uint256 day_) external nonReentrant {
        ...
        for(uint i=0;i<refundList.length;i++){
            ...
            if(refundOrder.state == 0){
                if(_investor[refundOrder.addr].orderList[refundOrder.orderId].refundState != 1){
                    continue;
                }
                _investor[refundOrder.addr].orderList[refundOrder.orderId].refundState = 2;
                _refundList[day_][i].state = 1;
                emit RefundDoneEvent(refundOrder.addr,refundOrder.orderId,refundOrder.amount);
                _usdt.safeTransferFrom(_msgSender(), refundOrder.addr, refundOrder.amount);
            }
        }
    }
